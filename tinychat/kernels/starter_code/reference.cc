        ...
        for (int col = 0; col < n; col++) {
            ...
            for (int ch = 0; ch < k;) {
                ...
                for (int qj = 0; qj < 32; qj++) {
                    ...
                    // each w_int4[qj] is an 8bit (w_i, w_[i+32]) pairing from above (for example (w0, w32))
                    // the first w_de_0 is taking the lower 4 bits, but in an 8bit representation, and then 
                    // subtracting 8 and making a signed char get us in the value in the range of (-8, 7) 
                    // the second w_de_16 is taking the higher 4 bits (by shifting right 4 bits), but in an 8bit representation, and then
                    // subtracting 8 and making a signed char get us in the value in the range of (-8, 7) 
                    // see AWQ paper p6 for more:  https://arxiv.org/pdf/2306.00978
                    uint8_t packed_int4_0 = w_int4[qj];                 
                    signed char w_de_0 = (packed_int4_0 & 0x0F) - 8.0;  
                    signed char w_de_16 = (packed_int4_0 >> 4) - 8.0;   
                    
                    // int8 multiply and accumulate operation
                    // activations are in int8 so thats why we converted weight in int4 -> int8 for this operation 
                    // since we have unpacked w_i + 32 as well, we perform both multiplications here with the 
