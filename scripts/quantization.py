...
def pseudo_quantize_model_salient_weight_fp16(model, w_bit, q_group_size, input_feat):
    for n, m in model.named_modules():
        if isinstance(m, nn.Linear):
            ...
            # element having dimension 2048 (e.g. features dim)
            # so here we sum across one feature for all 127 samples
...
def pseudo_quantize_model_random_weight_fp16(model, w_bit, q_group_size, input_feat):
    for n, m in model.named_modules():
        if isinstance(m, nn.Linear):
            ...
            num_samples = int(0.01 * importance.shape[0])
            outlier_mask = torch.randint(0, importance.shape[0], (num_samples,))
...
def pseudo_quantize_model_weight_scaleup(
    ...
    for n, m in model.named_modules():
        if isinstance(m, nn.Linear):
            ...
            # To simulate applying the scale factor, we can simply multiply
            # it before quantization, and then divide by the scale factor
            # after quantization.
...
def auto_scale_block(module, name, w_bit, q_group_size, input_feat):
    ...
    def _search_module_scale(block, linears2scale: list, x, kwargs={}):
        ...
        x: torch.Tensor = x.to(next(block.parameters()).device)
        with torch.no_grad():
            # calculate the operation * input (e.g. WX)
            org_out = block(x, **kwargs)
            if isinstance(org_out, tuple):
                org_out = org_out[0]

        # average magnitude of activation per channel
        s_x = x.view(-1, x.shape[-1]).abs().mean(0)
        assert torch.isnan(s_x).sum() == 0
        ############### YOUR CODE STARTS HERE ###############
        ...
        for ratio in range(n_grid):
            ...
            # now we compute the output based on the scaled / quantized W
            out = block(x, **kwargs)
